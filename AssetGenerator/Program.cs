...
namespace AssetGenerator
{
    internal partial class Program
    {
        ...
        /// </summary>
        public static readonly Regex EndNumberFinder = GenerateEndNumberFinder();
        ...
        private static string SanitizeMemberName(string assetName) => assetName.Replace('-', '_').Replace(' ', '_');
        ...
        private static string ProcessAssetContents(List<string> pngPaths, List<string> oggPaths, string className, string relativePath)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("namespace TwilightEgress.Assets");
            sb.AppendLine("{");
            ...
            sb.AppendLine($"{Indent}[GeneratedCode(\"{toolHeader}\", \"{versionHeader}\")]");
        ...
        private static void ProcessTextureContents(StringBuilder sb, List<string> pngPaths, string relativePath)
        {
            ...
            foreach (var group in pathGroups.OrderBy(g => g.Key))
            {
                ...
                foreach (string path in group.OrderBy(p => p))
                {
                    ...
                    sb.AppendLine($"{Indent}{Indent}{Indent}{Indent}public static readonly Asset<Texture2D> {SanitizeMemberName(assetName)} = ModContent.Request<Texture2D>(\"{pathRelativeToSource}\");");
                }
        ...
        {
            sb.AppendLine($"{Indent}{Indent}public class Sounds");
            sb.AppendLine($"{Indent}{Indent}{{");
            ...
            foreach (var group in pathGroups.OrderBy(g => g.Key))
            {
                ...
                foreach (string path in group.OrderBy(p => p))
                {
                    ...
                    sb.AppendLine($"{Indent}{Indent}{Indent}{Indent}public static readonly SoundStyle {SanitizeMemberName(assetName)} = new SoundStyle(\"{pathRelativeToSource}\"{variantSyntax});");
        ...
        private static partial Regex GenerateEndNumberFinder();
    }
}